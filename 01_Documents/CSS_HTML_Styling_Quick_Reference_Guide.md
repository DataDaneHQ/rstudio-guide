...
For guidance on linking a stylesheet to your report see the [R Markdown Quick Reference Guide - YAML Header](RMD_Quick_Reference_Guide.md#yaml-header), .
...
Defines the font, size, weight, and colour for each heading level in your report — ensuring a consistent visual hierarchy from h1 down to h4.
...
Controls the colour and underline behaviour of all clickable links in your report. By default browsers render links in blue with an underline — this overrides both to match your colour palette.
...
Controls the appearance of two distinct code elements — inline code (short snippets wrapped in single backticks within a sentence) and code blocks (multi-line chunks wrapped in triple backticks). Keeps both visually consistent and clearly distinguishable from regular body text.
...
Controls the appearance of all tables in your report — including header row colour, cell padding, row striping, and hover highlighting. Overrides the browser's default table styling to match your colour palette and ensure consistency across every table without needing to style each one individually.
...
Utility classes are small, single-purpose CSS classes you apply directly to any element in your report using class="class-name". Rather than writing inline styles repeatedly throughout your document, you define them once in style.css and reuse them anywhere — keeping your code clean and consistent.
